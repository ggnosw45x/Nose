...
@Client.on_message(filters.command(["revip"], ["/", "."]))
async def register(_, m: Message):
    with open(file='plugins/rangos/admins.txt',mode='r+',encoding='utf-8') as archivo:
        ...
        if str(m.from_user.id) + '\n' in x:
            data = m.text.split(" ", 2)
            if len(data) < 2:
                await m.reply_text("<b>⎚ Usar <code>/revip id</code></b>")
                return
            ...
            with open('plugins/rangos/vip.txt', 'w') as f:
                f.writelines(lineas)

                await m.reply(f'<b>𝙇𝙄𝙎𝙏𝙊 𝙀𝙇 𝙐𝙎𝙐𝘼𝙍𝙄𝙊 <code>{hola}</code> 𝘼 𝘿𝙀𝙅𝘼𝘿𝙊 𝘿𝙀 𝙎𝙀𝙍 𝙋𝙍𝙀𝙈𝙄𝙐𝙈 𝙀𝙓𝙄𝙏𝙊𝙎𝘼𝙈𝙀𝙉𝙏𝙀✅</b>')
        else:
            return await m.reply(f'<b>⎚𝙎𝙤𝙡𝙤 𝙡𝙤𝙨 𝙖𝙙𝙢𝙞𝙣𝙞𝙨𝙩𝙧𝙖𝙙𝙤𝙧𝙚𝙨 𝙥𝙪𝙚𝙙𝙚𝙣 𝙪𝙨𝙖𝙧 𝙚𝙨𝙩𝙚 𝙘𝙤𝙢𝙖𝙣𝙙𝙤❌</b>')
